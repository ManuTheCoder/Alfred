...
client.on("guildMemberAdd", (member) => {
  member.send(
    `Welcome to the server! To get a list of commands, type  \`-a help\`.`
  );
});
...
@R22 Gamer#8398`
    ...
    case "-a anime pat":
      ...
      msg.channel.send(embed).then((sent) => {
        let id = sent.id;
        fetch(
          "https://some-random-api.ml/animu/" +
            msg.content.replace("-a anime ", ""),
          { method: "Get" }
        )
          .then((res) => res.json())
          .then((json) => {
...
`
        ...
        .setFooter(
          'If you want to skip a field, just type "skip", and type the | character'
        )
        .addField(
          "Where do I find a hex code? ",
          'Search up "Color Picker" on google, pick your color, and copy the HEX code!'
        );
      msg.channel.send(embed);
      break;
    case "-a help":
    ...
    case "-a cmds":
      function createNewEmbed() {
        const embed = new Discord.MessageEmbed()
          ...
          .setFooter(
            "Want more features? Email me at: manuthecoder@protonmail.com"
          )
          .setThumbnail(
          ...
          .addField("Pictures", `\`-a help pictures\``)
...
`;
    ...
    case "-a image generator triggered":
      var link =
        "https://some-random-api.ml/canvas/" +
        encodeURIComponent(msg.content.replace("-a image generator ", "")) +
        "?avatar=" +
    ...
    case "-a pet actions":
      var embed = new Discord.MessageEmbed()
        ...
        .setTitle("Pet Actions").setDescription(`
    ...
    case "-a invite":
      msg.channel.send(
        new Discord.MessageEmbed()
          .setTitle("Invite me!")
          .setURL(
            "https://discord.com/api/oauth2/authorize?client_id=844303394335096862&permissions=8&scope=bot"
          )
          .setDescription(
            "https://discord.com/api/oauth2/authorize?client_id=844303394335096862&permissions=8&scope=bot"
          )
    ...
    default:
      ...
      if (msg.content.startsWith("-a rpc")) {
        ...
        var link =
          ...
          encodeURIComponent(msg.author.displayAvatarURL({ format: "png" })) +
          "&username=" +
          encodeURIComponent(msg.author.username) +
          "&comment=" +
          encodeURIComponent(
            msg.content.replace("-a image generator yt comment ", "")
          );
        msg.channel.send(new MessageAttachment(link, "img.gif"));
        ...
        var link =
          ...
          "&comment=" +
          encodeURIComponent(
            msg.content.replace("-a image generator tweet ", "")
          );
        msg.channel.send(new MessageAttachment(link, "img.gif"));
	...
	`;
                  ...
                  msg.channel.send(
                    new Discord.MessageEmbed()
                      ...
                      .addField(
                        "Note",
                        "If the video is longer than 2 hours, you might experience minor lag"
                      )
                  );
...
`;
        ...
        var title = msg.content.replace("-a embed", "").split("|")[0] || "";
        var desc = msg.content.replace("-a embed", "").split("|")[1] || "";
        var color = msg.content.replace("-a embed", "").split("|")[2] || "";
        var thumb1 = msg.content.replace("-a embed", "").split("|")[3] || "";
        var img1 = msg.content.replace("-a embed", "").split("|")[4] || "";
        var embed = new Discord.MessageEmbed()
          .setTitle(title)
          .setDescription(desc)
          .setColor(color);
...
Template Name List:
	`);
        ...
        if (topText && bottomText && template) {
          var image = `https://apimeme.com/meme?meme=${template}&top=${encodeURIComponent(
            topText
        ...
        if (msg.member.hasPermission("ADMINISTRATOR")) {
          if (msg.content.replace("-a cn ", "").toString().length > 32) {
            msg.channel.send("Username must be 32 or fewer in length!");
            return;
          }
          ...
          msg.channel.send(
            new Discord.MessageEmbed()
              ...
              .setDescription(
                ":white_check_mark: Successfully changed nickname to " +
                  msg.content.replace("-a cn ", "")
              )
              .setColor("#ffffff")
          );
        }
        ...
        fetch(
          "https://api.giphy.com/v1/gifs/search?q=Happy+Birthday&api_key=" +
...
client.on("ready", () => {
  client.user
    .setActivity("to -a help", { type: "LISTENING" })
    .then((presence) =>
